...
class MoE(nn.Module):
    ...
    def forward(self, x):
        ...
        sample_indices = torch.arange(batch_size, device=device)[:, None].expand(-1, self.top_k).flatten()
        ...
        for expert_idx in range(self.num_experts):
            ...
            outputs.index_add_(0, expert_samples, weight_output)
...
if __name__ == "__main__":
    ...
    for _ in range(5):
        ...
        print("Training output shape:", output.shape, "Auxiliary loss:", aux_loss.item())

    moe.eval()
    output, aux_loss = moe(x)
    print("Evaluation output shape:", output.shape, "Auxiliary loss:", aux_loss)
